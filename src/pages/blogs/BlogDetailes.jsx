...
const Blogs = () => {
  ...
  const { data, isLoading, refetch, isError, error } = useFetch(
    `/api/blogs/${params.blogRef}`,
    params.blogRef
  );

  const { fetchData, loading, isErr, err } = useSend();
  ...
  const deleteBlogHandler = async () => {
    ...
    toast({
      title: isErr ? err : response.message,
  ...
  const handleComment = async (e) => {
    e.preventDefault();
    if (isAuth) {
      const res = await fetchData(`/api/comments/${blogId}`, "POST", {
        description: comment,
      });
      toast({
        title: res.message,
  ...
  const likeHandler = async () => {
    if (isAuth) {
      ...
      res?.success && setLike(res.liked);
      refetch();
    } else {
      toast({
        title: "Please login to like this blog.",
        description: "You need to be logged in to like this blog.",
      });
    }
  };
  return (
    <div className="flex flex-col items-center mt-28">
      <Helmet>
        ...
        <meta
          name="description"
          content={data ? data.description.substring(0, 160) : "Blog Not Found"}
        />
        <link
          rel="canonical"
          href={`https://legitblogs.me/blogs/${params.blogRef}`}
        />
        <script type="application/ld+json">
          {JSON.stringify({
            "@context": "https://schema.org",
            "@type": "BlogPosting",
            headline: data ? data.title : "Blog Not Found",
            description: data
              ? data.description.substring(0, 160)
              : "Blog Not Found",
            author: {
              "@type": "Person",
              name: data ? data.author : "Unknown",
            },
            datePublished: data ? data.createdAt : "Unknown",
            image: data ? data.img.url : "",
            url: `https://legitblogs.me/blogs/${params.blogRef}`,
          })}
        </script>
      </Helmet>
      <motion.div
